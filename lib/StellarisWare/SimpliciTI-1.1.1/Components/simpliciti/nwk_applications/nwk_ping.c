...
static volatile uint8_t sPingTid = 0;
  ...
  do
  {
...
#else
    {
      ...
      if (SMPL_SUCCESS == SMPL_Ioctl(IOCTL_OBJ_RAW_IO, IOCTL_ACT_READ, &ioctl_info.recv))
      {
        ...
        sPingTid++;   /* guard against duplicates */
      }
    }
  } while (repeatIt);
...
static void smpl_send_ping_reply(mrfiPacket_t *frame)
{
   ...
   */
  pOutFrame = nwk_buildFrame(SMPL_PORT_PING, MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS, MRFI_GET_PAYLOAD_LEN(frame)-F_APP_PAYLOAD_OS, MAX_HOPS);
  if (pOutFrame)
  {
...
fhStatus_t nwk_processPing(mrfiPacket_t *frame)
{
   ...
   */
  replyType = nwk_isValidReply(frame, sPingTid, PB_REQ_OS, PB_TID_OS);
