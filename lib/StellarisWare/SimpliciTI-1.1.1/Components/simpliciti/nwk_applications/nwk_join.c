...
static          uint8_t (*spJoinCallback)(linkID_t) = NULL;
static volatile uint8_t  sJoinTid = 0;
...
void nwk_joinInit(uint8_t (*pf)(linkID_t))
{
  ...
  spJoinCallback = pf;
  (void) spJoinCallback;  /* keep compiler happy if we don't use this */

  sJoinTid = MRFI_RandomByte() ;
...
static void smpl_send_join_reply(mrfiPacket_t *frame)
{
  ...
  if (pOutFrame)
  {
    ...
    {
      if (nwk_saveJoinedDevice(frame) && spJoinCallback)
      {
        spJoinCallback(0);
      }
    }
...
smplStatus_t nwk_join(void)
{
  ...
  numChan=nwk_scanForChannels(channels);
  if (!numChan)
  {
...
#else
  {
    ...
    if (SMPL_SUCCESS == SMPL_Ioctl(IOCTL_OBJ_RAW_IO, IOCTL_ACT_READ, &ioctl_info.recv))
    {
         ...
         )
      {
        ...
        sJoinTid++;   /* guard against duplicates */
        rc = SMPL_SUCCESS;
#if defined( FREQUENCY_AGILITY )
        break;
#endif
...
fhStatus_t nwk_processJoin(mrfiPacket_t *frame)
{
   ...
   */
  if (SMPL_MY_REPLY == (replyType=nwk_isValidReply(frame, sJoinTid, JB_REQ_OS, JB_TID_OS)))
  {
