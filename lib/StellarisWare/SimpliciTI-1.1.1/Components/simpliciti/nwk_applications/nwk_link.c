...
static volatile uint8_t  sLinkTid = 0;
...
static void smpl_send_unlink_reply(mrfiPacket_t *frame)
{
  ...
  pCInfo = nwk_findPeer((addr_t *)MRFI_P_SRC_ADDR(frame), *(MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS+UL_RMT_PORT_OS));
  if (pCInfo)
  {
...
#endif
    pOutFrame = nwk_buildFrame(SMPL_PORT_LINK, msg, sizeof(msg), MAX_HOPS-(GET_FROM_FRAME(MRFI_P_PAYLOAD(frame),F_HOP_COUNT)));
    if (pOutFrame)
    {
      /* destination address is the source adddress of the received frame. */
      memcpy(MRFI_P_DST_ADDR(&pOutFrame->mrfiPkt), MRFI_P_SRC_ADDR(frame), NET_ADDR_SIZE);
...
fhStatus_t nwk_processLink(mrfiPacket_t *frame)
{
   ...
   */
  if (SMPL_MY_REPLY == (replyType=nwk_isValidReply(frame, sLinkTid, LB_REQ_OS, LB_TID_OS)))
  {
