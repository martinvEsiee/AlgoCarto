...
BufferCallback(void *pvBuffer, unsigned long ulEvent)
{
    ...
    if(ulEvent & 0x00000001)
    {
        ...
        {
            g_ulFlags |= 0x00000002;
        }

        
        
        
        g_ulBytesPlayed += 4096 >> 1;
    }
}
...
WaveOpen(unsigned long *pulAddress, tWaveHeader *pWaveHeader)
{
    ...
    if((pulBuffer[0] != 0x46464952) || (pulBuffer[2] != 0x45564157))
    {
    ...
    if(((4096 >> 1) / ulBytesPerSample) > 1024)
    {
        g_ulMaxBufferSize = 1024 * ulBytesPerSample;
    }
    else
    {
        g_ulMaxBufferSize = 4096 >> 1;
    }
...
WavePlayStart(tWaveHeader *pWaveHeader)
{
    ...
    g_ulFlags = 0x00000001 | 0x00000002;
...
WavePlayContinue(tWaveHeader *pWaveHeader)
{
    ...
    if(g_ulFlags & 0x00000002)
    {
        
        
        
        ulCount = WaveRead(pWaveHeader, &g_pucBuffer[4096 >> 1]);

        
        
        
        SoundBufferPlay(&g_pucBuffer[4096 >> 1],
    ...
    if((ulCount < g_ulMaxBufferSize) || (g_ulBytesRemaining == 0))
    {
        ...
        while(g_ulFlags != (0x00000002 | 0x00000001))
        {
        }
...
{
    return((g_ulFlags & 0x00000004) ? 1 : 0);
}
